...
document.querySelectorAll('.skill-card, .project-card, .stat, .contact-method').forEach(el => {
    observer.observe(el);
});
...
contactForm.addEventListener('submit', function(e) {
    ...
    const whatsappMessage = `Olá Matheus! Meu nome é ${name} e gostaria de entrar em contato sobre um projeto. Email: ${email}. Mensagem: ${message}`;
    const whatsappUrl = `https://wa.me/5543984336883?text=${encodeURIComponent(whatsappMessage)}`;
...
// Removed parallax effect to prevent sections from moving with scroll
...
document.addEventListener('keydown', (e) => {
    ...
    if (konamiCode.join(',') === konamiSequence.join(',')) {
        showNotification('🎉 Konami Code ativado! Você encontrou o easter egg!', 'success');
...
console.log('🚀 Portfólio do Matheus Cardoso carregado com sucesso!');
